                    ...
                    <div class="col-md-12">
                        ...
                        <div class="box box-primary">
                            ...
                            <div class="box-body">
                                ...
                                <a data-toggle="modal" data-target="#modal_add" class="btn btn-primary" >Add Slide Show</a> 
                                        ...
                                        <?php
                                            ...
                                            foreach ($data as $dkey => $dval) {
                                                echo '<tr>';
                                                    ...
                                                    echo '<td>' . $dval->url . '</td>';
                                                    echo '<td>' . $dval->description . '</td>';
                                                    echo '<td>' . $dval->status . '</td>';
                                                    echo '<td>';
                                                        ?>  
                                                            <a href="" data-toggle="modal" data-target="#modal_edit" class="btn btn-warning" onclick="modal_edit('<?php echo $dval->title ?>', '<?php echo $dval->id ?>', '<?php echo $dval->description ?>', '<?php echo $dval->status ?>', '<?php echo $dval->url ?>')">Edit</a>
    ...
    <script type="text/javascript">
        ...
        function modal_edit(name, id, description, status, image) {
            $('#input-edit-id').val(id);
            $('#input-edit-title').val(name);
            $('#input-edit-description').val(description);

            $("input[name=edit_status][value=" + status + "]").prop('checked', true);

            $('#input-edit-gambar').val(image);
        }

        function modal_delete(name, id, image) {
            $('#modal-delete-title').html(name);
            $('#modal-delete-name').html(name);
            $('#modal-delete-id').val(id);
            $('#modal-delete-url').val(image);
        }
